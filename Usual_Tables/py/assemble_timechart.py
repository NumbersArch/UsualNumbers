...
rnames, rsexes, rids = interpretText(vpath)
...
for d in alldir:
	...
	for p in paths:
		...
		if elementname in cnames:
			sx = "?"
			if elementname in rnames:
				idex = rnames.index(elementname)
				sx = rsexes[idex]
			elementname = elementname + " > "+sx
...
def orderDF(df):
	...
	df[["Company","Agency","Gen","Name", "Sex"]] = df["Name"].str.split(' > ',expand=True)
	df = df.sort_values(by=["Company","Agency","Gen","Sex","Name"], ascending=True)
	df.set_index(["Company","Agency","Gen","Sex","Name"], inplace=True)
	df = df.reset_index()
	return df
